...
| 规则配置 | `StrategyConfig` | 当前 35 个参数值 |
...
| `get_rule_engine()` | 只读 | `FunnelService.get_rule_engine()` |
| `propose_rule_patch(diff_payload)` | 写提案 | → `agent_proposals` 表 |
...
| 页面 | 联动内容 |
|------|---------|
| 规则引擎 | 参数旁显示 `💡 Hermes 建议: 0.18 → 0.22`，点击跳转审批 |
...
- [ ] Hermes 能读取漏斗、公告、规则、同步日志等现有模块数据
...
| 非通用聊天入口 | Hermes 是面向投研和规则进化的专用 agent |
