...
_RISK_TEMPLATE = {"low": "turquoise", "medium": "yellow", "high": "red"}
_TYPE_LABEL = {
    "rule_patch": "策略规则",
...
def _format_diff_brief(diff_payload: dict | None, limit: int = 6) -> str:
    """把 diff_payload 压缩成 1-N 行可读的关键字段变更。"""
    if not diff_payload or not isinstance(diff_payload, dict):
        return ""
    lines: list[str] = []

    # 识别常见 diff 结构：{"before": {...}, "after": {...}} 或扁平字段
    before = diff_payload.get("before") if isinstance(diff_payload.get("before"), dict) else None
    after = diff_payload.get("after") if isinstance(diff_payload.get("after"), dict) else None

    if before and after:
        keys = list(after.keys())[:limit]
        for k in keys:
            b = before.get(k, "—")
            a = after.get(k, "—")
            if b != a:
        ...
        for k, v in list(diff_payload.items())[:limit]:
...
async def notify_proposal_created(proposal: dict) -> dict:
    """Hermes 产出新提案时推送。"""
    risk = proposal.get("risk_level") or "medium"
    r_emoji = _RISK_EMOJI.get(risk, "🟡")
    r_tpl = _RISK_TEMPLATE.get(risk, "yellow")
    type_label = _TYPE_LABEL.get(proposal.get("type", ""), proposal.get("type", ""))
    confidence = int(float(proposal.get("confidence", 0) or 0) * 100)
    title = proposal.get("title", "新策略提案")

    diff_brief = _format_diff_brief(proposal.get("diff_payload"))
    reasoning = (proposal.get("reasoning") or "").strip()
    if len(reasoning) > 160:
        reasoning = reasoning[:160] + "…"

    builder = (
        CardBuilder(title=f"💡 新策略提案 · {title}", template=r_tpl)
        .add_kv_inline(
            [
                ("类型", type_label),
                ("风险", f"{r_emoji} {risk}"),
    ...
    if reasoning:
        builder.add_markdown(f"**推理**\n{reasoning}")
    if diff_brief:
        builder.add_markdown(f"**变更**\n{diff_brief}")

    builder.add_note(f"提案 #{proposal.get('id', '-')} · 待审批")
    return await send_feishu_card(builder.build())


async def notify_proposal_decided(proposal: dict, action: str, note: str = "") -> dict:
    """提案批准/驳回后推送。action: approve | reject"""
    is_approve = action == "approve"
    template = "green" if is_approve else "grey"
    emoji = "✅" if is_approve else "🚫"
    action_label = "已批准" if is_approve else "已驳回"
    title = proposal.get("title", "策略提案")
    type_label = _TYPE_LABEL.get(proposal.get("type", ""), proposal.get("type", ""))

    builder = (
        CardBuilder(title=f"{emoji} 提案{action_label} · {title}", template=template)
        .add_kv_inline(
            [
                ("类型", type_label),
                ("操作", action_label),
    ...
    if is_approve:
        diff_brief = _format_diff_brief(proposal.get("diff_payload"))
        if diff_brief:
            builder.add_markdown(f"**已应用变更**\n{diff_brief}")
    if note:
        builder.add_markdown(f"**备注**\n{note}")

    builder.add_note("Alpha · 提案审批")
    return await send_feishu_card(builder.build())
