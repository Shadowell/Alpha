...
| **热门智能 Loop** | 60s 检查 / 5min 执行 | 检查热门股票智能分析是否过期，按配置自动刷新 Top20 逐股评分 |
...
- **自定义策略中心**（重点，取代原"缩量启动"区块）：将选股逻辑抽象为"规则组合"，用户可自由启用 / 关闭单条规则并调参
...
│       ├── hot_stock_ai_service.py    # 热门股智能分析（Top20·逐股打分·三池拆分）
...
| GET | `/api/strategy/hot-stock-ai` | 热门股票智能分析快照（Top20 逐股打分 + 三池结果） |
| POST | `/api/strategy/hot-stock-ai/run` | 手动触发热门股票智能分析 |
| GET | `/api/strategy/hot-stock-ai/config` | 读取热门股票智能分析配置 |
| POST | `/api/strategy/hot-stock-ai/config` | 更新热门股票智能分析配置 |
