...
- **自动同步**：每日 15:20 自动补最新交易日缺口，优先使用 AkShare `stock_zh_a_spot_em()` 全市场批量行情一次写入；历史全量补缺仅由手动按钮触发，单股抓取限时失败快跳过，完成类消息仅写日志
...
K 线同步完成类消息只写入服务日志，不再推送飞书，避免盘后补缺任务产生过多通知。
...
默认打开浏览器访问 http://127.0.0.1:18888；如果 18888 被不可用的旧进程占用，`start.sh` 会自动落到 18892-18899 的空闲端口，并在启动输出里打印实际访问地址。

### 环境变量

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `PORT` | `18888` | 服务端口；默认端口被不可用进程占用时自动使用 18892-18899 兜底 |
| `HOST` | `0.0.0.0` | 监听地址 |
