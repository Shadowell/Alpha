...
| sample_count | 1 | 单路径采样（多路径取均值可降低方差） |

#### 测试方法

每个模型独立加载，预热推理 1 次后执行 3 次有效推理，取耗时均值。模型之间完全释放内存。

#### 性能对比

| 模型 | 参数量 | 加载耗时 | 推理耗时 (avg) | D1 预测涨跌 | D5 预测涨跌 | 预测波动率 |
|------|--------|---------|---------------|------------|------------|-----------|
...
> - 预测结果含随机采样特性，每次运行会有差异，表中数值仅代表单次运行结果
> - D1/D5 预测涨跌 = (预测收盘价 - 历史最后收盘价) / 历史最后收盘价 × 100%
> - 预测波动率 = 预测期内日均 (high - low) / low × 100%

#### 结论

- **推理速度**：三个模型推理均在亚秒级（0.16s ~ 0.27s），日 K 场景下 MPS 加速效果显著
- **加载代价**：Kronos-base 首次加载约 3s（含模型下载缓存），之后常驻内存不再重复加载
- **参数量 vs 耗时**：small (24.7M) 与 base (102.3M) 参数量差 4 倍，但推理耗时仅差 0.02s，瓶颈在自回归逐步生成而非模型宽度
