...
- **缩量启动形态扫描**（重点）：前 N 天（默认 25）缩量横盘（振幅 ≤ 20% / 量能 CV ≤ 0.4）+ 最新一天放量涨停（量 ≥ 3 倍均量 + 涨停），全 A 股 5000 只约 30 秒扫完
  - 参数可前端调整：横盘天数、振幅、CV、放量倍数、是否必须涨停
  - 独立放大区块（橙红渐变边框 + 14px 圆角 + 260px 卡片网格），点击命中卡片直接弹出 Kronos 预测 modal（无跳转/无刷新）
  - 即便股票不在漏斗池中，也能正常加载基础信息 + 180 日 K 线（`/api/stock/{symbol}/detail` 退化兜底）
...
- **提案管理 Tab**：
  ...
  - 顶部 **Stats 概览**：待审批 · 今日新增 · 已批准 · 已驳回 · 近 7 日通过率
  - **筛选 + 搜索**：按状态 / 类型 / 风险等级过滤，支持关键词匹配标题或推理
  - **批量审批**：多选 + 批量批准/驳回，一次处理多条提案
  - **决策 Modal**：批准/驳回前展示变更 diff，附备注 textarea，避免误触
  - 16:00 自动追踪提案效果（对比漏斗变化），写入 Agent 长期记忆
  - 新提案产出 / 审批结果自动推送飞书卡片（见下文 飞书卡片推送）
...
| **C** | 策略回测实验室 | "缩量启动"策略在全 A 股 × 180 天历史上回测，返回胜率/累计收益/最大回撤（示例：576 信号 / 46% 胜率 / +390% 累计 / 87 秒） |
| **D** | 深度个股研报 | LLM 聚合"K线 + Kronos 预测 + 概念 + 公告"生成 200 字研报卡（含 verdict/confidence/利好/风险/操作建议） |
| **E** | 提案学习闭环 | 把最近 30 天 outcome_tracking 统计注入 daily_review 的 LLM prompt，让 Agent 主动规避"净增样本 < 0"的提案类型 |
...
| **新策略提案** | Hermes 产出提案 | 按风险 🟢/🟡/🔴 着色 · 类型/风险/置信度 · 推理摘要（≤160 字）· 关键 diff 字段 `before → after` |
| **提案审批结果** | 用户批准/驳回 | 🟢 批准 / ⚫ 驳回 · 操作人备注 · 已应用的变更（仅批准） |
...
| **通知** | 飞书 Webhook **交互式卡片**（K 线同步 / 预测选股 / 新提案 / 审批结果 / 周报） |
...
| GET | `/api/strategy/quiet-breakout` | 缩量启动形态最近一次扫描快照 |
| POST | `/api/strategy/quiet-breakout/scan` | 触发全 A 股扫描（参数：`lookback_days`/`amp_threshold`/`vol_cv_threshold`/`vol_spike_ratio`/`require_limit_up`） |
...
| GET  | `/api/hermes-ai/proposal-learner` | 提案效果学习统计 + 可注入 prompt 的片段 |
...
| GET | `/api/agent/proposals` | 提案列表（支持 `status/type/risk_level/q` 筛选 + `limit/offset`） |
| GET | `/api/agent/proposals/stats` | 提案看板：待审批/今日新增/已批准/已驳回/近 7 日通过率 |
| GET | `/api/agent/proposals/{id}` | 提案详情 + 反馈记录 |
| POST | `/api/agent/proposals/{id}/approve` | 批准提案（推飞书卡片） |
| POST | `/api/agent/proposals/{id}/reject` | 驳回提案（推飞书卡片） |
| POST | `/api/agent/proposals/batch` | 批量审批/驳回：`{ids:[...], action:"approve"/"reject", note?}` |
| POST | `/api/agent/proposals/create` | 创建提案（MCP/直连，自动推飞书卡片） |
...
`tests/test_api_regression.py` — 覆盖 50+ 个端点，分 12 大类：
...
| Hermes Agent | 7 | status / proposals 列表+stats / tasks / 监控 |
| Hermes AI 扩展 | 6 | risk / auto-trade / backtest / learner / news / weekly |
| 模拟盘 | 5 | positions / history / summary / trades / settings |
| 提案生命周期 | 4 | batch 校验 + 不存在 id skip + 404 |
| 公告详情 | 1 | 容忍 200/404 |
| 性能巡检 | 6 | 关键只读端点 < 3s |
...
- 缩量启动区块视觉尺寸（宽 ≥ 600px、高 ≥ 140px）
- **核心 bug 回归**：点击缩量启动卡片不触发页面跳转 / 刷新 / `window.open`，且必须正确弹出 Kronos 预测 modal
- 数据中心任务历史表格 grid cell 无水平 overlap（容差 2px）
- 提案管理子 tab 进入后 `#proposalStats` 渲染
- 模拟盘主容器渲染
